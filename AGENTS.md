...
- Этот файл описывает рабочие правила для агентных правок в проекте
...
- Для SCSS:
  ...
  - для брейкпоинтов использовать миксины из `src/scss/tools/_media.scss` (например, `@include mq.down(sm)`), а не прямые `@media` для мобильного брейкпоинта;
  - не группировать все мобильные стили в один общий медиа-блок в конце файла: размещать мобильные правила рядом с селекторами, к которым они относятся.
...
- Главное правило: не использовать фиксированные `height`/`min-height` для визуальной подгонки под макет. Совпадение с Figma достигать через корректную структуру, отступы, типографику, сетку и поведение контента.
- Главное правило: фиксированные `width` использовать по минимуму и только при реальной необходимости. По умолчанию использовать гибкие размеры (`auto`, `%`, `max-width`, поведение контейнера).
...
- Основная сетка проекта: `8` колонок, `gap: 2rem`.
- По умолчанию не делать адаптивную верстку. Любые мобильные или адаптивные стили добавлять только если об этом явно попросили в задаче.
- Использовать масштаб: `1rem = 10px`.
- Если значение равно `1px`, писать `1px`, а не `0.1rem`.
...
- Если `img` позиционируется абсолютно, не использовать `inset: 0`; использовать `left: 0; top: 0; width: 100%; height: 100%`.
